...
void init()
{
	my_memset(display_buf, 0 , display_size);
}
...
static inline void
draw_character_2(char ch, int x, int y, int color) {
	int i, j;
	char *p = font8x8_basic[(int)ch];
	for (i = 0; i < 8; i ++) 
		for (j = 0; j < 8; j ++) 
			if ((p[i] >> j) & 1){
				draw_pixel(x + 2*i, y + 2*j, color);
				draw_pixel(x + 2*i+1, y + 2*j, color);
				draw_pixel(x + 2*i+1, y + 2*j+1, color);
				draw_pixel(x + 2*i, y + 2*j+1, color);
				}
}
...
void
draw_big_string(const char *str, int x, int y, int color) {
	while (*str) {
		draw_character_2(*str ++, x, y, color);
		if (y + 16 >= 320) {
			x += 16; y = 0;
		} else {
			y += 16;
